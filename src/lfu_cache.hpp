...
class LFUCache {
 public:
  ...
  LFUCache<KeyType, ValueType> &operator=(LFUCache<KeyType, ValueType> &&lhs) noexcept {
    if (this != &lhs) {
      using std::swap;
 ...
 private:
  ...
  struct LFUCacheNode {
    ...
    LFUCacheNode(KeyType k, ValueType v)
        : key(k), value(v), count(1), prev(nullptr), next(nullptr) {
    }
  };
  ...
  struct CountListNode {
    ...
    explicit CountListNode(size_t c)
        : count(c), prev(nullptr), next(nullptr), cache(nullptr) {
    }
  };
